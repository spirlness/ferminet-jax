"""
Simplified baseline benchmark script for FermiNet - no nested structure handling in this version
Measures: forward pass, MCMC sample, energy loss+grad, Adam update, full train step
"""
...
def run_baseline_benchmark(epochs=10, output_file="baseline.json"):
    """Run baseline benchmark measuring key components"""
    ...
    # Base trainer (not ExtendedTrainer for simpler benchmarking)
    ...
    @jax.jit
    def forward_jit(params, r_batch):
        orig = network.params
    ...
    # Create log_psi function for MCMC
    ...
    for epoch in range(epochs + 1):
        ...
        # 2. MCMC sampling
        t0 = time.perf_counter()
        r_elec_new, _ = mcmc.sample(log_psi_fn, r_elec, key)
        jax.block_until_ready(r_elec_new)
        t_mcmc = time.perf_counter() - t0
        r_elec = r_elec_new
        key, _ = random.split(key)

        # 3. Energy loss and gradients
        t0 = time.perf_counter()
        ...
        res = trainer.train_step(params, r_elec, key, nuclei_pos, nuclei_charge)
        ...
        if not is_warmup:
            ...
            timings["energy_grad"].append(t_energy)
