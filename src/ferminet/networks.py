...
def _masked_mean(values: Array) -> Array:
    """Compute masked mean over axis=1, excluding the diagonal (self-interactions).

    Args:
        values: Array of shape (n, n, feat).
    """
    summed = jnp.sum(values, axis=1)
    diag = jnp.diagonal(values, axis1=0, axis2=1).T
    denom = values.shape[1] - 1
    return (summed - diag) / max(denom, 1.0)
