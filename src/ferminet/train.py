...
def train(cfg: ml_collections.ConfigDict) -> Mapping[str, Any]:
    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % checkpoint_every == 0:
            host_p, host_o, host_d = jax.device_get((params, opt_state, data))
            _last_host_params = jax.tree_util.tree_map(
                lambda x: x[0] if getattr(x, "ndim", 0) > 0 else x, host_p
            )
            _last_host_opt_state = jax.tree_util.tree_map(
                lambda x: x[0] if getattr(x, "ndim", 0) > 0 else x, host_o
            )
            _last_host_data = jax.tree_util.tree_map(
                lambda x: x[0] if getattr(x, "ndim", 0) > 0 else x, host_d
            )
            _last_ckpt_step = i + 1
