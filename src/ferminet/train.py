    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % print_every == 0:
            ...
            if stats_host.ndim == 1:
                stats_first = stats_host
            else:
                stats_first = stats_host[0]

            energy_val = float(stats_first[0])
            variance_val = float(stats_first[1])
            pmove_val = float(stats_first[2])
            lr_val = float(stats_first[3])
        ...
        if (i + 1) % adapt_frequency == 0:
            if stats.ndim == 1:
                ...
                pmove_arr = stats[:, 2]
            pmove_value = _to_host(pmove_arr)
