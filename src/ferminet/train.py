...
def train(cfg: ml_collections.ConfigDict) -> Mapping[str, Any]:
    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % checkpoint_every == 0:
            host_params, host_opt_state, host_data = jax.device_get(
                (params, opt_state, data)
            )
            _last_host_params = jax.tree_util.tree_map(
                lambda x: x[0] if getattr(x, "ndim", 0) > 0 else x, host_params
            )
            _last_host_opt_state = jax.tree_util.tree_map(
                lambda x: x[0] if getattr(x, "ndim", 0) > 0 else x, host_opt_state
            )
            _last_host_data = jax.tree_util.tree_map(
                lambda x: x[0] if getattr(x, "ndim", 0) > 0 else x, host_data
            )
            _last_ckpt_step = i + 1
