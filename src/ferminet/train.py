    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % print_every == 0:
            ...
            if stats_host.ndim == 2:
                stats_flat = stats_host[0]
            else:
                stats_flat = stats_host

            energy_val = _to_float(stats_flat[ENERGY])
            variance_val = _to_float(stats_flat[VARIANCE])
            pmove_val = _to_float(stats_flat[PMOVE])
            lr_val = _to_float(stats_flat[LEARNING_RATE])
