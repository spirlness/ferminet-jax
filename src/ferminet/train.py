...
def train(cfg: ml_collections.ConfigDict) -> Mapping[str, Any]:
    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % print_every == 0:
            ...
            if not jnp.isfinite(energy_val):
                ...
                log_stats = train_utils.StepStats(
                    energy=energy_val,
                    variance=_convert_to_float(stats_host[VARIANCE]),
                    pmove=_convert_to_float(stats_host[PMOVE]),
                    learning_rate=_convert_to_float(stats_host[LEARNING_RATE]),
                )
        ...
        if (i + 1) % adapt_frequency == 0:
            pmove_value = _convert_to_float(jax.device_get(stats[PMOVE]))
