...
def train(cfg: ml_collections.ConfigDict) -> Mapping[str, Any]:
    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % print_every == 0:
            ...
            # Extract elements from the tuple, handling potential sharded arrays (e.g. from pmap)
        ...
        # Handle potential sharded stats tuple elements
        pmove_ref_el = stats[PMOVE]
        if hasattr(pmove_ref_el, "ndim") and pmove_ref_el.ndim > 0:
            pmove_ref = pmove_ref_el[0]
        else:
            pmove_ref = pmove_ref_el
