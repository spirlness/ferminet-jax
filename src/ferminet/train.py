...
def train(cfg: ml_collections.ConfigDict) -> Mapping[str, Any]:
    ...
    # Indices for packed stats
    idx_energy, idx_variance, idx_pmove, idx_lr = 0, 1, 2, 3
    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % print_every == 0:
            ...
            energy_val = _to_float(stats_host[..., idx_energy])

            log_stats = train_utils.StepStats(
                energy=energy_val,
                variance=_to_float(stats_host[..., idx_variance]),
                pmove=_to_float(stats_host[..., idx_pmove]),
                learning_rate=_to_float(stats_host[..., idx_lr]),
            )
        ...
        if (i + 1) % adapt_frequency == 0:
            pmove_value = _to_host(stats[..., idx_pmove])
