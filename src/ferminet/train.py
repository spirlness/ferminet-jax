...
def train(cfg: ml_collections.ConfigDict) -> Mapping[str, Any]:
    ...
    for i in range(step, iterations):
        ...
        # Fetch stats as a single contiguous array to host to avoid redundant D2H syncs
        ...
        if (i + 1) % print_every == 0:
            ...
            train_utils.log_stats(i + 1, log_stats, wall, width)
            start = time.time()
        elif not jnp.isfinite(energy_val):
            # Must still reset width and continue on non-finite energies
