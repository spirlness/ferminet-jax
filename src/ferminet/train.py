...
def train(cfg: ml_collections.ConfigDict) -> Mapping[str, Any]:
    ...
    if cfg_any.optim.optimizer == "kfac":
        ...
        def kfac_step_fn(
            ...
            def _first(x):
                return jnp.reshape(jnp.asarray(x), (-1,))[0]

            energy = _first(loss_value)
            variance = _first(aux.variance)
            pmove_val = _first(pmove)
            step_val = _first(step)
            lr = jnp.asarray(schedule(step_val))

            stats = jnp.stack([energy, variance, pmove_val, lr])
    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % adapt_frequency == 0:
            if stats.ndim == 2:
                pmove_val = stats[0, PMOVE]
            else:
                pmove_val = stats[PMOVE]
