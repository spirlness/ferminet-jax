...
def train(cfg: ml_collections.ConfigDict) -> Mapping[str, Any]:
    ...
    for i in range(step, iterations):
        ...
        if (i + 1) % print_every == 0:
            ...
            # Check shape to ensure stats were stacked correctly.
            # Expected shape: (n_devices, 4) for pmapped Adam, or (4,) for KFAC/unbatched.
            if stats_host.ndim == 2:
                stats_host = stats_host[0]
            # Ensure we have 4 metrics: energy, variance, pmove, lr
            assert stats_host.shape[-1] == 4
        ...
        if (i + 1) % adapt_frequency == 0:
            if stats.ndim == 2:
                pmove_value = _to_host(stats[0, PMOVE])
            else:
